...
@profile
def calculate_flo2d_model_accuracy(exec_datetime, obs_station_list, model='FLO2D', version='250', forward=2, backward=3,
                                   sim_tag='hourly_run'):
    # print('calculate_flo2d_rule_accuracy|execution_date : ', exec_datetime)
    # print('calculate_flo2d_rule_accuracy|sim_tag : ', sim_tag)
    ...
    if len(obs_station_list) > 0:
        for [obs_station, allowed_error] in obs_station_list:
            station_error = calculate_station_accuracy(obs_adapter, fcst_adapter,obs_station, model, version, exec_datetime, backward, sim_tag)
            print('calculate_flo2d_model_accuracy|station_error : ', station_error)
            print('calculate_flo2d_model_accuracy|allowed_error : ', allowed_error)
            if station_error is not None:
                if station_error <= allowed_error:
                    print('{} is accurate'.format(obs_station))
                    station_result[obs_station] = True
        ...
        accuracy_percentage = (success_count / total_stations) * 100
        print('calculate_flo2d_rule_accuracy|accuracy_percentage : ', accuracy_percentage)
    obs_adapter.close_connection()
    fcst_adapter.close_connection()


def calculate_station_accuracy(obs_adapter, fcst_adapter, obs_station, flo2d_model, flo2d_version,
                               exec_datetime, observed_days, sim_tag, method='RMSE'):
    ...
    # print('calculate_station_accuracy|obs_station_id : ', obs_station_id)
    if obs_station_id is not None:
        ...
        if obs_df is not None:
            ...
            # print('calculate_station_accuracy|cell_id : ', cell_id)
...
def get_cell_id(station_name, model, version, fcst_adapter=None):
    # print('get_cell_id|station_name : ', station_name)
    ...
    if cell_map is not None:
        ...
        # print('get_cell_id|channel_cell_map : ', channel_cell_map)
        # print('get_cell_id|flood_plain_cell_map : ', flood_plain_cell_map)
        for cell_id, cell_station_name in channel_cell_map.items():
            if station_name == cell_station_name:
                return cell_id
        for cell_id, cell_station_name in flood_plain_cell_map.items():
            if station_name == cell_station_name:
                return cell_id
    return cell_id
...
def get_matching_flo2d_station(obs_station, cell_id, fcst_adapter=None):
    ...
    print('get_matching_flo2d_station|fcst_station_name : ', fcst_station_name)
    if fcst_adapter is None:
        fcst_adapter = get_curw_fcst_adapter()
    flo2d_station_id = fcst_adapter.get_flo2d_station_id_by_cell_id1(cell_id)
    if flo2d_station_id is not None:
        print('get_matching_flo2d_station|flo2d_station_id : ', flo2d_station_id)
        return flo2d_station_id
    else:
        flo2d_station_id = fcst_adapter.get_flo2d_station_id_by_cell_id2(cell_id)
        return flo2d_station_id
...
# def get_latest_fgt(hash_id, start_time, fcst_adapter=None):
#     print('get_latest_fgt|[hash_id, start_time] : ', [hash_id, start_time])
#     if fcst_adapter is None:
#         fcst_adapter = get_curw_fcst_adapter()
#
#     fcst_adapter.get_latest_fgt(hash_id, start_time)
#     fcst_adapter.connection.cursor().callproc('getLatestFGTs', [hash_id, start_time])
#     for result in fcst_adapter.connection.cursor().stored_results():
#         print('get_latest_fgt|result : ', result.fetchall())
#     rows = fcst_adapter.connection.cursor().fetchall()
#     print('get_latest_fgt|rows : ', rows)
#     if rows is not None:
#         if rows:
...
def get_fcst_tms(flo2d_station_hash_id, exec_datetime, tms_start, tms_end, fcst_adapter=None):
    if fcst_adapter is None:
        fcst_adapter = get_curw_fcst_adapter()
    latest_fgt = fcst_adapter.get_latest_fgt(flo2d_station_hash_id, exec_datetime)
    # latest_fgt = get_latest_fgt(flo2d_station_hash_id, tms_start, fcst_adapter)
    tms_df = fcst_adapter.get_station_tms_fgt(flo2d_station_hash_id, tms_start, tms_end, latest_fgt)
...
def get_obs_tms(obs_station_hash_id, exec_datetime, tms_start, tms_end, obs_adapter=None):
    if obs_adapter is None:
        obs_adapter = get_curw_obs_adapter()
    tms_df = obs_adapter.get_timeseries_by_id(obs_station_hash_id, tms_start, tms_end, 15)
    if tms_df is not None:
...
if __name__ == "__main__":
    ...
    exec_datetime = '{} {}'.format(run_date, run_time)
    # exec_datetime = '2020-06-21 08:00:00'
    observed_stations = [['Janakala Kendraya', 5], ['Kaduwela Bridge', 5], ['Ingurukade', 5],
                         ['Diyasaru Uyana', 5], ['Yakbedda', 5]]
    obs_db_config = {'mysql_user': 'admin', 'mysql_password': 'floody', 'mysql_host': '35.197.98.125',
                     'mysql_db': 'curw_obs', 'log_path': '/home/hasitha/PycharmProjects/DSS-Framework/log'}
    sim_db_config = {'mysql_user': 'admin', 'mysql_password': 'floody', 'mysql_host': '35.197.98.125',
                     'mysql_db': 'curw_sim', 'log_path': '/home/hasitha/PycharmProjects/DSS-Framework/log'}
    fcst_db_config = {'mysql_user': 'admin', 'mysql_password': 'floody', 'mysql_host': '35.197.98.125',
                      'mysql_db': 'curw_fcst', 'log_path': '/home/hasitha/PycharmProjects/DSS-Framework/log'}
    calculate_flo2d_model_accuracy(exec_datetime, observed_stations)
