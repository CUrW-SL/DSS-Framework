...
docker run -i --rm --privileged -v /mnt/disks/data/samba/wrf-static-data/geog:/home/Build_WRF/geog curw-wrfv4:ubuntu1604  /home/Build_WRF/code/wrfv4_run.sh -d ${START_DATE} -h ${HOUR} -c ${CHECK_GFS}  -s ${VERSION} -r ${RUN} -m ${MODEL} -u ${GFS_URL} -v ${OUTPUT_DIR}:/home/Build_WRF/nfs  -v ${GFS_DIR}:/home/Build_WRF/gfs -v ${ARCHIVE_DIR}:/home/Build_WRF/archive