...
class CurwFcstAdapter:
    ...
    def get_wrf_fcst_station_ids(self):
    ...
    def get_wrf_source_id(self, wrf_model, wrf_version):
        model = 'WRF_{}'.format(wrf_model)
        sql_query = 'select id from curw_fcst.source where model=\'{}\' and version=\'{}\''.format(model, wrf_version)
        result = self.get_single_result(sql_query)
        if result is not None:
            station_id = result[0]
            return station_id
    ...
    def get_wrf_station_hash_id(self, sim_tag, source_id, station_id):
        sql_query = 'select id from curw_fcst.run where sim_tag=\'{}\' and station={} and ' \
                    'source={} and variable=1 and unit=1'.format(sim_tag, station_id, source_id)
        result = self.get_single_result(sql_query)
        if result is not None:
            station_hash_id = result[0]
            return station_hash_id
        else:
            return None

    def get_stations_time_series(self, hash_id, fgt_limits):
        sql_query = 'select time,value from curw_fcst.data where id=\'{}\' and ' \
                    'fgt>\'{}\' and fgt<\'{}\''.format(hash_id, fgt_limits[0], fgt_limits[1])
        print('get_station_tms|sql_query : ', sql_query)
        results = self.get_multiple_result(sql_query)
        if results is not None:
